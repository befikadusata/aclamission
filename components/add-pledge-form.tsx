...
export function AddPledgeForm({ onSuccess, onCancel }: AddPledgeFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("pledges").insert([pledgeData])
      ...
      toast({
        title: "Success",
        description: "Pledge has been added successfully.",
      })
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to add pledge",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex gap-4 justify-end">
        ...
        <Button type="submit" disabled={isLoading}>
          {isLoading ? "Adding Pledge..." : "Add Pledge"}
        </Button>
      </div>
    </form>
  )
